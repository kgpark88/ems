...
# 테스트 데이터에 대해 모델을 평가하고 점수를 출력한다.
score, _ = model.evaluate(Xtest, Ytest, batch_size=BATCH_SIZE)
rmse = math.sqrt(score)
print("\nMSE: {:.3f}, RMSE: {:.3f}".format(score, rmse))

plt.plot(scaler.inverse_transform(Ytrain), 'r-', label="Power Usage")
plt.plot(scaler.inverse_transform(model.predict(Xtrain[:,:,:])), 'b-', label="Prediction")
plt.xlim(0, 96*5)
plt.legend()
